...
local function initializeIfNeeded()
    ...
    mp.add_forced_key_binding(":", "seek-to-start-position", function()
        mp.commandv("seek", startPosition, "absolute")
        mp.command("show-progress")
        updateTrimmingPositionsOSDASS()
    end)
    mp.add_forced_key_binding('"', "seek-to-end-position", function()
...
local function generateDestinationPath()
    ...
    local output = filename .. "_$n." .. extension
...
-- Static Key Bindings
--
mp.add_key_binding(";", "trim-set-start-position", setStartPosition)
mp.add_key_binding("'", "trim-set-end-position", setEndPosition)
